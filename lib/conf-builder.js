...
const loadConf = rootDir => {
  ...
  try {
    ...
    confFiles.forEach( confFile => {
      const confFileName = path.join(rootDir, '_res', confFile);
...
const resolveSetting = (connection) => {
  ...
  Object.keys(conf).forEach(confKey => {
    Object.keys(connection).forEach(connectionKey => {
      const connectionValue = connection[connectionKey];
      const resolvedValue = connectionValue.toString().replace("${"+confKey+"}", conf[confKey]);
      connection[connectionKey] = resolvedValue;
    })
  })
}
